...
const CARD_TOOLTIPS = {
  ...
  bottleneck: {
    text: "User-authored chokepoint topics. Each topic names a demand driver, lists the upstream layers that physically constrain it, and carries a thesis card per stock. Anchors are the obvious capex spenders, shown unranked as the demand trace. Underdogs are emerging stocks with great potential: smaller names, filtered by a per-topic market-cap ceiling (default $3B) and ranked by 40-day ROC. The thesis card carries the potential read; momentum is a stress gauge (BOTTLENECK_LOOKBACK_DAYS), not a signal. Topics can be created, edited, generated by the drafting agent, and imported or exported as JSON.",
