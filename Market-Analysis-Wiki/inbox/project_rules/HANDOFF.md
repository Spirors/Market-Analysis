...
`Last updated`: 2026-09-13 22:30 UTC (Wiki-centric architecture deployed
— claude-obsidian adopted; 99 sources ingested; project-rules skill
removed (hard-rules inlined into AGENTS.md); originals relocated from
`project_rules/` and `docs/` at the repo root to
`Market-Analysis-Wiki/raw/` preserving original directory structure.)

**Note:** the original canonical markdown is now in `Market-Analysis-Wiki/inbox/`
(the `raw/` duplicate was removed in a follow-up — see the 2026-09-13
SESSION_LOG entry for context). All Session Start paths in AGENTS.md
have been redirected to `Market-Analysis-Wiki/inbox/`.

## Current state

**Architecture is now wiki-centric.** `Market-Analysis-Wiki/` is the
canonical knowledge base, managed by the claude-obsidian skill set.
Source documents (former `project_rules/*.md` and `docs/**/*.md`) live
at `Market-Analysis-Wiki/inbox/<dir>/<file>.md` (preserving the
original directory structure). Wiki pages at
`Market-Analysis-Wiki/wiki/sources/<slug>.md` cite each source back
to its canonical path + SHA-256 + immutable capture at
`.raw/captured/<sha>.md`. AGENTS.md points Session Start paths at
`Market-Analysis-Wiki/inbox/project_rules/` and
`Market-Analysis-Wiki/inbox/docs/`.

**Vault state:** 99 source pages ingested (9 from `project_rules/`
root, 26 from `archive/sessions/`, 47 from `archive/decisions/`, 17
from `docs/{logs,superpowers}/`). Captures live at
`.raw/captured/<sha>.md` (content-addressed, idempotent — re-running
on unchanged sources is a no-op). `wiki/index.md` lists all 99 sources
grouped by type; `wiki/log.md` has the ingest entry; `wiki/hot.md`
has updated recent facts.

**WSL infrastructure:** Ubuntu 22.04 + Python 3.12.13 installed;
`/etc/wsl.conf` has `[automount] options = "metadata"` so chmod works
on `/mnt/c/`. claude-obsidian writes require WSL (POSIX dirfd
confinement); reads can be native.

**AGENTS.md** was rewritten to inline all Core rules (data integrity,
frozen files, commit hygiene, shared components, file ownership,
session continuity, process hygiene, test isolation, documentation
hygiene, wiki sync) and drop the `## Skills` pointer section. The
`project-rules` skill was removed entirely — its content has moved
into AGENTS.md where every session can read it without a `skill` tool
call.

**Skill installation:** `bash scripts/setup-multi-agent.sh --host
opencode --apply` installed 15 claude-obsidian skill symlinks at
`~/.config/opencode/skills/<name>` (autoresearch, canvas, defuddle,
obsidian-bases, obsidian-markdown, save, think, wiki, wiki-cli,
wiki-fold, wiki-ingest, wiki-lint, wiki-mode, wiki-query,
wiki-retrieve). The 8 pre-existing skills were untouched.

## Top 3 next actions

1. **Run `wiki-lint` on the vault to confirm the 99-source ingest
   passes deterministic validation.** Should be a no-op since the
   ingest bundle was vetted before apply, but worth confirming end-to-end.
2. **Push the local commits** (`AGENTS.md` rewrite + `.gitignore`
   `Market-Analysis-Wiki/inbox/` line + deleted `project_rules/` and
   `docs/` directories) to origin/main. Currently 3 commits ahead of
   `origin/main` plus all the new untracked deletions.
3. **Run wiki-ingest to fold any further edits to
   `Market-Analysis-Wiki/inbox/**/*.md`** into the vault.
   (HANDOFF/SESSION_LOG are operational, not knowledge — they don't
   need wiki-ingest themselves.)

## Blockers

None. `data/events.json` has unstaged scheduler timestamp updates —
per `RUNBOOK.md` the `MarketAnalysis-EventsCommit` task owns that
file, not interactive sessions, so they will be picked up at the next
17:00 scheduled run.

## Notes for the next session

- **Wiki writes require WSL.** `os.name == "nt"` fails closed in
  claude-obsidian (`_require_write_platform` → `UNSUPPORTED_PLATFORM`).
  Use `wsl -d Ubuntu-22.04` and the python at
  `/mnt/c/Users/Spirors/AppData/Local/Temp/opencode/claude-obsidian/scripts/claude-obsidian.py`.
- **Junctions in `inbox/` are forbidden by capture's SOURCE_SYMLINK
  check.** The capture pipeline refuses reparse points. Use physical
  copies (already `.gitignore`d at `Market-Analysis-Wiki/inbox/`).
- **Source-ledger source IDs must be `src-<sha-prefix>`, not the raw
  SHA256.** `stable_source_id(origin_kind, locator, content_sha256)`
  in `claude_obsidian/ledgers.py:407` generates the correct format.
  Use `source_manifest_updates` in the transaction bundle instead of
  writing `wiki/meta/ledgers/source-ledger.json` directly — the
  engine manages it from the manifest updates.
- **The vault itself is untracked by git** (no explicit rule, just
  nothing tracked under `Market-Analysis-Wiki/`). Future sessions
  should keep the wiki out of git; `inbox/` inside the wiki is the
  canonical markdown store, not a git working copy.
- **AGENTS.md is now ~249 lines** (up from 141). The hard-rules
  inlining was the dominant contributor (~190 lines). Per the
  Documentation Hygiene rule, this is acceptable for a "read in
  full, every time" file. Re-check after any future major edits.

---


`Last updated`: 2026-09-11 19:25 UTC (Portfolio follow-up fixes ΓÇö
...
ΓÇö AI proxies cooldown is unchanged. (2) Γû▓/Γû╝ reorder now mirrors the
...
persistence + per-section refresh cooldowns ΓÇö 6 commits across two
parallel lanes. Backend: new `POST /api/portfolios/<pid>/holdings/reorder`
endpoint + `app.config.REFRESH_SECTION_COOLDOWNS` gating
`refresh_market()` on cached `vintage` stamps. Frontend: Γû▓/Γû╝ persists
via the new endpoint, Γû▓/Γû╝ greyed out when `sort.key !== "default"`,
`applyCooldownBadge` + refresh-button hover tooltip showing
"Last refresh: X min ago ΓÇö Next refresh available in: N min".
...
`archive/sessions/2026-09-11-portfolio-reorder-persistence-refresh-cooldowns.md`. (Note: the Portfolio portion of the per-section cooldown was removed later same day; see the follow-up entry above and DECISIONS.md "Portfolio enrichment always runs (no cooldown skip) ΓÇö 2026-09-11".)
...
9 ΓåÆ 7 (NOT 8 ΓÇö user caught the miscount; the 8th strategy
...
**Earlier (2026-09-10):** News section overhaul ΓÇö two follow-up
...
auto-refreshes on tag edits ΓÇö the user clicks the global Refresh
button to pick up their changes. Backend: 459 passed (+16). Frontend
news section: 12 passed (+5). `data/events.json` NOT committed ΓÇö
...
holdings row reorder + "Γå║ Default order" restored.)
...
**Γû▓/Γû╝ greyed out when view is in column-header sort** (commit
`b0e9789`). Buttons render with `disabled` + tooltip `"Reset to
default order (Γå║) before reordering rows"` when `sort.key !== "default"`.
`moveRow` also has a defensive guard.

**Per-section refresh cooldowns** (commits `5e00482` + `040c409`).
Portfolio: 15 min (`vintage["portfolios"]`). Breadth ΓÇö AI proxies:
...
live `"Last refresh: X min ago ΓÇö Next refresh available in: N min"`
title on the global `#refreshBtn` on hover.

**News timeline ΓÇö every pill is editable for manual fix** (commit
`fa2c976`, follow-up to `4734cc9`). Two popover modes:

- **Fixed-dimension pills** (category / actor / direction / region).
  Click ΓåÆ popover shows a `<select>` of valid values for that dimension
...
- **User-added / `ai` tags.** Click ΓåÆ popover shows the existing
  free-text rename input + Remove. Saving POSTs to `/api/events/tags`.

The AI capex-cycle gauge does NOT auto-refresh on tag edits ΓÇö only the
global Refresh button triggers the recompute (which happens via
`/api/dashboard` ΓåÆ `_enrich` ΓåÆ `_recompute_ai_sentiment`). The tag-edit
endpoint no longer returns `ai_sentiment`.

**Bottleneck section is interactive** ΓÇö still green from prior session.
Γåæ / Γåô chevrons + Γ£Ä rename pencil, prefs in `data/bottleneck_prefs.json`,
canonical `BOTTLENECK_CATEGORIES` untouched.

**From prior sessions ΓÇö still green and unaffected:**

- **P0 perf fix** (commit `25e5c06`). 3 s add/delete latency ΓåÆ <500 ms
...
- **P1** (server lifecycle) and **P2** (data integrity) cleared by the
  audit.
- **Audit follow-up closure** ΓÇö every P3..P7 follow-up item from
...
2. **Module-graph discipline** ΓÇö when extending events.js Γåö cards.js
   imports, do NOT add a `?v=ΓÇª` query to one side without the other.
   The JS spec creates a fresh module record per URL; a mismatch
   silently duplicates the module and splits its state (this is
   exactly what bit the news overhaul ΓÇö see the entry in
   `project_rules/DECISIONS.md` and the verification log in
   `archive/sessions/2026-09-10-news-section-overhaul-ΓÇªmd`).
3. **Archive `data/logs/summary-2026-09-11.md`** (gitignored daily
   changelog) once the session is well past ΓÇö these files grow fast
   and are local-only per `AGENTS.md`. Not urgent.

## Blockers

None. `data/events.json` has unstaged scheduler timestamp updates ΓÇö
...
- **Per-section refresh cooldowns are gated on `vintage` stamps in
  `data/dashboard.json`** ΓÇö there is no separate `data/refresh_state.json`.
  ...
  (`breadth-ai` card ΓåÆ `indicators` vintage) is intentionally
  asymmetric ΓÇö the card's displayed age is `vintage.indicators` per
...
- **Holdings reorder persistence uses `app.portfolio.reorder_holdings`
  ...
  Γû▓/Γû╝ click handler in `tickerTable.js:moveRow` is optimistic ΓÇö swap
...
- **The `cooldown_skip` field is always present in the dashboard
  ...
  refreshed X min ago ΓÇö next refresh in N min"` ΓÇö N is computed live
  ...
  live in **two** places ΓÇö `app.config` for the server-side gate and
  `static/js.cards.applyCooldownBadge` for the badge / tooltip. If
  one changes, change both.

- **Global `#refreshBtn` tooltip is now dynamic** ΓÇö on hover it shows
...
- **Test isolation fixture (`tests/conftest.py`) did NOT need a new
  monkeypatch line for the cooldown logic** ΓÇö the existing
...
- **`app/validation.py` is the only surviving earnings artifact.** It's
  ...
  `app/earnings.py` ΓÇö keeps the yfinance history-primary /
...
- **Columns dropdown lives INSIDE the expanded portfolio** now
  (`controlsMode: 'columnsOnly'`). The card-header dropdown is gone ΓÇö
...
- **The Portfolio rename CSS shift fix lives in JS, not CSS.** The
  ...
  sizing into CSS alone will re-introduce the shift ΓÇö CSS can't
...
- **Default pytest run is the full suite now.** Last session the
  ...
  Both are now cleaned up and pass ΓÇö the audit found the
