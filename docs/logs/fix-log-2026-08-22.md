...
These were resolved on 2026-08-23 (branch `parked/review-2026-08-23`, one commit per item — see `docs/park-log.md`): things that would change analysis outputs (breadth double-count of SMH+SOXX, verdict semantics, missing-vs-zero handling), add features (regime expiry), change infrastructure (cross-process locking), or decide security posture (endpoint auth), plus the Stooq mapping check (fallback turned out to be entirely dead and was removed).
