...
- **English-edition RSS feeds** — MarketWatch, BBC Business (High/Critical events, 48h window only; cross-source dedupe merges same-story items). Tune `IMPORTANCE_THRESHOLD` in `app/config.py` to surface more items on quiet days.
